...
function MobileNav({ watchlist, selectedStock, onSelect }) {
  ...
  return (
    <motion.nav
      initial={{ y: 100 }}
      animate={{ y: 0 }}
      className="fixed bottom-0 left-0 right-0 bg-terminal-bg-secondary/95 backdrop-blur-xl border-t border-terminal-border z-40"
    >
      <div className="flex items-center justify-around py-2 px-4">
        {navItems.map((item) => (
          ...
          >
            ...
            {item.badge && (
              <span className="absolute -top-1 -right-1 w-5 h-5 bg-terminal-green text-terminal-bg text-xs rounded-full flex items-center justify-center">
                {item.badge}
              </span>
            )}
            {activeTab === item.label.toLowerCase() && (
